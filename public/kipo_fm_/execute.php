    ...
    switch($_GET['action'])
    {
        ...
        case 'rename_folder':
            if ($rename_folders){
                ...
                if (!empty($name)){
					...
					{
						response(_trans('Rename_existing_folder'), 403)->send();
						exit;
					}
                    ...
                    response(_trans('Empty_name'), 400)->send();
					exit;
                }
            }
            break;
        case 'create_file':
            if ($create_text_files === FALSE) {
                response(sprintf(_trans('File_Open_Edit_Not_Allowed'), strtolower(_trans('Edit'))), 403)->send();
				exit;
            }
            ...
            if (strpos($name, '.') === FALSE){
                response(_trans('No_Extension').' '.sprintf(_trans('Valid_Extensions'), implode(', ', $editable_text_file_exts)), 400)->send();
				exit;
            }
            ...
            if (!in_array(end($parts), $editable_text_file_exts)) {
                response(_trans('Error_extension').' '.sprintf(_trans('Valid_Extensions'), implode(', ', $editable_text_file_exts)), 400)->send();
				exit;
            }
            ...
            // file already exists
            if (file_exists($path)) {
                response(_trans('Rename_existing_file'), 403)->send();
				exit;
            }
            ...
            if (@file_put_contents($path, $content) === FALSE) {
                response(_trans('File_Save_Error'), 500)->send();
				exit;
            }
            else {
                ...
                response(_trans('File_Save_OK'))->send();
				exit;
            }
        ...
        case 'paste_clipboard':
            ...
            if (is_really_writable($path) === FALSE || is_really_writable($path_thumb) === FALSE){
                response(_trans('Dir_No_Write').'<br/>'.str_replace('../','',$path).'<br/>'.str_replace('../','',$path_thumb), 403)->send();
				exit;
            }

            // check if server disables copy or rename
            if (is_function_callable(($action == 'copy' ? 'copy' : 'rename')) === FALSE){
                response(sprintf(_trans('Function_Disabled'), ($action == 'copy' ? lcfirst(_trans('Copy')) : lcfirst(_trans('Cut')))), 403)->send();
				exit;
            }
        ...
        case 'chmod':
            ...
            // check perm
            if ($chmod_perm === FALSE) {
                response(sprintf(_trans('File_Permission_Not_Allowed'), (is_dir($path) ? lcfirst(_trans('Folders')) : lcfirst(_trans('Files')) )), 403)->send();
				exit;
            }

            // check mode
            if (!preg_match("/^[0-7]{3}$/", $mode)){
                response(_trans('File_Permission_Wrong_Mode'), 400)->send();
				exit;
            }
            ...
            if (is_function_callable('chmod') === FALSE){
                response(sprintf(_trans('Function_Disabled'), 'chmod'), 403)->send();
				exit;
            }
