...
if(isset($_GET['action']))
{
    switch($_GET['action'])
    {
		...
		case 'save_img':
			...
			if ($image_data === false)
			{
				response(_trans('Aviary_No_Save'), 400)->send();
				exit;
			}
		...
		case 'extract':
			...
			switch ($info['extension'])
			{
				case "zip":
					...
					{
						response(_trans('Zip_No_Extract'), 500)->send();
						exit;
					}
				...
				default:
					response(_trans('Zip_Invalid'), 400)->send();
					exit;
			}
			break;
		case 'media_preview':
		...
		case 'copy_cut':
			...
			if (is_dir($path))
			{
				...
				{
					response(sprintf(_trans('Copy_Cut_Not_Allowed'), ($_POST['sub_action'] == 'copy' ? lcfirst(_trans('Copy')) : lcfirst(_trans('Cut'))), _trans('Folders')), 403)->send();
					exit;
				}
				...
				if ($copy_cut_max_size !== false && is_int($copy_cut_max_size))
				{
					...
					{
						response(sprintf(_trans('Copy_Cut_Size_Limit'), ($_POST['sub_action'] == 'copy' ? lcfirst(_trans('Copy')) : lcfirst(_trans('Cut'))), $copy_cut_max_size), 400)->send();
						exit;
					}
				}
				...
				if ($copy_cut_max_count !== false && is_int($copy_cut_max_count))
				{
					...
					{
						response(sprintf(_trans('Copy_Cut_Count_Limit'), ($_POST['sub_action'] == 'copy' ? lcfirst(_trans('Copy')) : lcfirst(_trans('Cut'))), $copy_cut_max_count), 400)->send();
						exit;
					}
				}
			}
			else
			{
				...
				{
					response(sprintf(_trans('Copy_Cut_Not_Allowed'), ($_POST['sub_action'] == 'copy' ? lcfirst(_trans('Copy')) : lcfirst(_trans('Cut'))), _trans('Files')), 403)->send();
					exit;
				}
			}
		...
		case 'chmod':
			...
			{
				response(sprintf(_trans('File_Permission_Not_Allowed'), (is_dir($path) ? lcfirst(_trans('Folders')) : lcfirst(_trans('Files'))), 403), 400)->send();
				exit;
			}
			else
			{
				...
				if (is_dir($path))
				{
					$ret .= '<div>'._trans('File_Permission_Recursive').'
							<ul>
								<li><input value="none" name="apply_recursive" type="radio" checked> '._trans('No').'</li>
								<li><input value="files" name="apply_recursive" type="radio"> '._trans('Files').'</li>
								<li><input value="folders" name="apply_recursive" type="radio"> '._trans('Folders').'</li>
								<li><input value="both" name="apply_recursive" type="radio"> '._trans('Files').' & '._trans('Folders').'</li>
							</ul>
							</div>';
				}
		...
		case 'get_lang':
			...
			{
				response(_trans('Lang_Not_Found'), 404)->send();
				exit;
			}
		...
		case 'get_file': // preview or edit
			...
			{
				response(_trans('File_Not_Found'), 404)->send();
				exit;
			}
			...
			{
				response(sprintf(_trans('File_Open_Edit_Not_Allowed'), ($sub_action == 'preview' ? strtolower(_trans('Open')) : strtolower(_trans('Edit')))), 403)->send();
				exit;
			}
